...
export default function App() {
  ...
  // State Kategori dengan LocalStorage
  const [kategoriList, setKategoriList] = useState(() => {
    const saved = localStorage.getItem("cdb_kategori");
    return saved
      ? JSON.parse(saved)
      : ["Nongkrong", "Touring Malam", "Mabar Game", "Projek Brutal", "Kopdar Rutin"];
  });
  ...
  const handleTambahKategori = (e: any) => {
    ...
    localStorage.setItem("cdb_kategori", JSON.stringify(updateList));
    setKategoriBaru("");
  };
